  ...
  return (
    <div className={`flex my-3 gap-3 ${alignLeft ? "" : "justify-end"} ${showTrending ? "justify-between flex-col sm:flex-row sm:items-center" : ""}`}>
      ...
      <div className="flex min-w-80 rounded-full bg-gray-800">
        {filters.map((f) => (
          <span className={`font-semibold py-2 w-1/3 text-center rounded-full cursor-pointer ${filter == f.value ? f.background : ""}`} key={f.title} onClick={() => setFilter(f.value)}>{f.title}</span>
        ))}
      </div>
    </div>
  );
